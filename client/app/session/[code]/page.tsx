...
function SessionPageInner() {
  ...
  const webrtc = useWebRTC(media.streamRef, media.setMediaError);
  const chat = useChat(sessionId, name, chatOpen);
  const canvas = useCanvas();

  useEffect(() => {
    async function start() {
      // IMPORTANT: initMedia FIRST so streamRef.current is set
      // before initSocket runs and peer-joined fires
  ...
  async function initSession() {
    ...
    // initSocket AFTER media is ready (initMedia already awaited above)
